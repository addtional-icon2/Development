...
# 누락값을 사용하고 싶으면 Missing_Data 테이블 추가
...
#urls.py에 의해 들온 사용자의 요청에 realtime.html템플릿을 실시간 검색 순위가 담긴 posts 변수와 날짜 값이 담긴 date, 시간 값이 담긴 time 변수, 누락값이 담긴 missing변수와 함께 보내준다.
...
#urls.py에 의해 들어온 사용자의 요청에 realtime.html템플릿을 실시간 검색 순위가 담긴 posts 변수와 날짜 값이 담긴 date, 시간 값이 담긴 time변수와 함께 보내준다.
                return render(request, 'crawling/index.html', {'posts': posts, 'date': userdate, 'time': usertime})
